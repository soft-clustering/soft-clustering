...
than dropped. Nine estimators are validated against an independent
implementation of the same algorithm or against the algorithm they provably
reduce to in a limit, with a largest membership disagreement of
$6.2\times10^{-10}$ and identical hard partitions throughout; the other 33
carry weaker, property-based evidence, and we say so.
...
42 comparisons). SCPP is documented, tested with 1{,}204 automated tests
...
\item \textbf{Validation against evidence outside SCPP.}
Nine estimators are checked against an independent implementation of the same
algorithm or against the algorithm they provably reduce to in a limit ---
\texttt{scikit-fuzzy}, \texttt{scikit-learn}, \texttt{networkx}, and
\texttt{evclust}, the reference package of the evidential methods' author.
The largest membership disagreement across every comparable pair is
$6.2\times10^{-10}$ and every hard partition is identical. Building these
checks exposed four defects that no internal test had caught
(Appendix~\ref{app:external}); we report the remaining 33 estimators as
having weaker, property-based evidence rather than describing the library as
uniformly validated.
...
1{,}204 automated tests at 95\% line coverage, CI across three operating
...
\paragraph{Testing.}
The suite contains 1{,}204 tests across 48 modules and achieves 95\% line
...
%% --- generated by the commands in paper/README.md ---
\begin{table}[t]
\centering
\small
...
\section{Validation Against Evidence Outside SCPP}\label{app:external}

Every correctness check in \S\ref{sec:quality} compares SCPP to SCPP:
invariants it declares for itself, and --- for the five optimised algorithms
of Appendix~\ref{app:optimization} --- agreement with its own preserved
earlier code. None of that can detect an implementation that is
self-consistently wrong. This appendix reports what we can establish from
outside, graded by strength, and states plainly how far it reaches.

\paragraph{Tier 1: agreement with an independent implementation.}
The strongest evidence short of a proof --- a second codebase, written by
other people from the same paper, reaching the same solution. Seven
estimators have one. Four are compared quantitatively in
Table~\ref{tab:external-baselines}: fuzzy c-means against
\texttt{scikit-fuzzy}, the Gaussian mixture against \texttt{scikit-learn},
evidential c-means against \texttt{evclust} --- which is maintained by the
author of the method, making a disagreement far more likely to be our error
than theirs --- and LDA against \texttt{scikit-learn}'s variational
implementation. Three more are checked against an independent computation of
the objective rather than of the fit: CDCGS and DMoN both maximise soft
modularity, and evaluated on a crisp assignment their objective must equal
the modularity \texttt{networkx} computes for that partition, which it does
to $6\times10^{-9}$; and SoftDBSCANGM's component count is determined by its
DBSCAN stage and is checked against \texttt{scikit-learn}'s DBSCAN across
...
Across every comparable pair the largest membership disagreement is
$6.2\times10^{-10}$ and every hard partition is identical. Where the two sides
do not define the same quantity we say so rather than manufacture a number:
\texttt{evclust} places residual evidential mass on the frame $\Omega$ while
SCPP places it on a noise cluster, so for ECM we compare prototypes
($1.4$--$2.2\times10^{-2}$) and the partition instead.

\paragraph{Tier 2: reduction to an independently implemented algorithm.}
Several methods provably degenerate to a simpler one in a limit, and the
limiting algorithm has a trusted implementation. Fuzzy c-means at $m = 1.02$
must reproduce k-means, and does: identical partitions and prototypes
agreeing to $10^{-3}$ with \texttt{scikit-learn}'s. A spherical Gaussian
mixture with vanishing regularisation must too, and does. Gustafson--Kessel's
fuzzy covariance is the identity on isotropic clusters, where it must match
fuzzy c-means. Possibilistic fuzzy c-means at zero typicality weight reduces
to Bezdek's objective. Each of these exercises the estimator's whole update
path against an outside implementation, and adds two estimators (GK, PFCM) to
the seven above.

\paragraph{Tier 3: properties implied by the definition.}
Weaker, but not vacuous, and available where no reference exists. A consensus
function must be idempotent --- the consensus of one partition repeated must
be that partition --- which SCSPA, SHBGF and SMCLA satisfy exactly. A
possibilistic typicality must be a pointwise function of the distance to one
prototype, so PCM's must be monotone in that distance and must not sum to one
across clusters. A mixture must recover the parameters that generated its
data, which MBMM does on Beta samples. Rough $k$-means must satisfy Lingras
and West's interval-set axioms: the lower approximation contained in the
upper, at most one lower approximation per object, and every object in at
least one upper approximation. MMSB, CDCGS, DMoN, FeMIFuzzy, Gath--Geva,
EVCLUS and PLSI must recover planted structure with a known answer.

\paragraph{What this covers, and what it does not.}
Nine of 42 estimators have Tier 1 or Tier 2 validation. The count is asserted
in the test suite rather than only stated here, so it cannot drift. For the
remaining 33 the evidence is Tier 3 plus the invariants and edge cases of
Appendix~\ref{Testing} --- enough to establish that they compute something
with the right structure and the right behaviour in the limits we can reason
about, not enough to establish that they compute what their paper specifies.
We regard closing that gap as the most valuable single addition to this
library, and the harness is written so that adding a pair is roughly a dozen
lines.

\paragraph{This is not a formality: it found four defects.}
Building this appendix changed the library. SCPP's Gaussian mixture seeded EM
with k-means++ \emph{centres} rather than a converged k-means solution and
settled at $n = 200$ on prototypes $(1.06, 1.73, 6.07)$ where the generating
means were $(0, 3, 6)$ --- ARI $0.50$ against \texttt{scikit-learn}; running
Lloyd's algorithm first, as \texttt{scikit-learn} does, closed the gap to ARI
$1.00$ and made the fit thirty times faster. LDA merged two of three cleanly
separated topics on two initialisations in ten, and drew its initialisation
from NumPy's global generator so that a fit could not be reproduced without
seeding it externally; it now takes a \texttt{random\_state} and keeps the
best of \texttt{n\_init} restarts by corpus log-likelihood, recovering all
three topics in ten of ten. Rough $k$-means assigned objects using absolute
radii derived from the inter-centroid distances rather than Lingras and
West's relative rule, so an object far from every centroid entered no
approximation at all and received an all-zero membership row --- a violation
of the method's defining axioms that no shape or invariant check had caught.
EVCLUS's stress surface proved flat enough that a single restart gave
stresses differing by a factor of two between runs that differed only in the
memory layout of the input. Three of the four are the same failure mode:
a non-convex objective, one initialisation, no restarts. None was visible
from inside SCPP.

\section{Testing}\label{Testing}

SCPP has \textbf{1{,}204 tests across 48 modules} at \textbf{95\% line
