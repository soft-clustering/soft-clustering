...
class DMoN(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, edge_index: torch.Tensor, adj: torch.Tensor
    ) -> torch.Tensor:
    ...
    def loss(self, soft_assign: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        ...
        modularity_term = torch.trace(soft_assign.T @ B @ soft_assign) / (2 * m + 1e-8)

        cluster_size = soft_assign.sum(dim=0)
        collapse_reg = (torch.norm(cluster_size, p=2) / soft_assign.shape[0] - 1) ** 2
